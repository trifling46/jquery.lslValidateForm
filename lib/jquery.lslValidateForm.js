...
function updateInputStatus(input,status) {
    ...
    if(validateType == 'ls-h-validateForm'){
        ...
        //var left = $(input)[0].offsetLeft+$(input).outerWidth()+hOffsetLeft; //tip紧跟当前input 右侧
        var left = $(input).parents("form").outerWidth()+hOffsetLeft;   //tip 在整个form右侧
...
function initFormValidate(form) {
    ...
    var validateType = $(form).dataset("validate-type");
    ...
    var validateObj = {
        ...
        validateForm:function () {
            inputs.each(function(index,input){
                var vtypes = $(input).data("validateObj").types;
                var _this = input ;
                $.each(vtypes,function (index,item) {
    ...
    $(form).data("validateObj",validateObj);
    ...
    inputs.each(function (index,input) {
        ...
        // 水平方法 input 父元素必须为relative 定位，tip提示才能准确定位
        if(validateType == 'ls-h-validateForm' ){
...
function ValidateObj(input) {
    this.input = input;
    this.msg = $(input).dataset("vmsg")?$(input).dataset("vmsg"):'输入有误';
...
function validateByType(input,type) {
    ...
    var value = $(input).val();
    switch (type){
        case 'required':status = validateType.noEmpty(input);break;  //required
        case 'phone':value && (status = validateType.phone(input));break;       //phone,只有在不为空的时候才参与验证（保证可为空）
        case 'email':value && (status = validateType.email(input));break;       //email
        case 'identityID':value && (status = validateType.identityID(input));break;       //identityID
        case 'fixedPhone':value && (status = validateType.fixedPhone(input));break;       //fixedPhone
        case 'regexp':value && (status = validateType.regexp(input));break;       //regexp
    }
    return status;
}
$(function () {
